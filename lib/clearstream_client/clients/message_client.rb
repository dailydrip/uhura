  ...
  class MessageClient < BaseClient
    ...
    def self.create_subscriber(data)
      ...
      begin
        ...
        msg = "Failed to create subscriber. Clearstream response: #{response.to_json}. APIError: #{e.message}"
        log_error(msg)
        return ReturnVo.new(value: nil, error: return_error(msg, :unprocessable_entity))
      end
      msg = "Requested Clearstream to create subscriber (#{new_subscriber_data.to_json}). Resonse: #{response.to_json}"
      log_info(msg)
