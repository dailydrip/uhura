...
class Sendgrid
  ...
  def self.send(message_vo)
    ...
    log_warn("Received empty body from Sengdrid for messsage_id (#{message_vo.message_id})") if response[:body].blank?
    # Record time message sent to Sendgrid. Later, populate with response time.
    sendgrid_msg = SendgridMsg.create!(sent_to_sendgrid: sent_to_sendgrid_at,
